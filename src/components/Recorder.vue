<template>
  <div class="main">
    ...
    <div class="mainBox">
      <div class="pd">
        类型：{{ type }}
        <span style="margin: 0 20px"> 比特率: <input type="text" v-model="bitRate" style="width: 60px" /> kbps </span>
        ...
        <div v-for="obj in logs" :key="obj.idx">
          <div :style="{ color: obj.color == 1 ? 'red' : obj.color == 2 ? 'green' : obj.color }">
            ...
            <template v-if="obj.res">
              {{ intp(obj.res.rec.set.bitRate, 3) }}kbps {{ intp(obj.res.rec.set.sampleRate, 5) }}hz 编码{{
    ...
    recStop: function () {
      ...
      rec.stop(
        function (blob, duration) {
          ...
          // TODO
          socket.emit('audio', { blob, id: 123123 })
        },
        function (s) {
